...
const i18n = {
  az: {
    ...
    about_p1: "2019-cu ildən Xəzər TV-də yayımlanan Komedixana, Azərbaycanın ən populyar komediya verilişlərindən biridir. Hər həftə milyonlarla izləyici bu şounu həm televiziyada, həm də YouTube-da izləyir.",
    about_p2: "Veriliş yumoristik səhnəciklər, satirik monoloqlar və aktual sosial mövzular üzərində qurulub. Teatr üslubunda hazırlanan hər bölüm tamaşaçılara gülüş, enerji və müsbət düşüncə bəxş edir.",
    ...
    ticket_desc: "KOMEDİXANA çəkilişinə canlı tamaşaçı kimi qatılın! Aşağıdakı formanı doldurun və biz sizinlə əlaqə saxlayaq.",
    form_name: "Ad Soyad",
    ...
    form_ok: "✅ Müraciətiniz qəbul edildi! Tezliklə sizinlə əlaqə saxlanılacaq.",
    opt_1: "1 nəfər",
    opt_2: "2 nəfər",
    opt_3: "3 nəfər",
    opt_4: "4 nəfər",
  ...
  ru: {
    ...
    about_p1: "«Komedixana» выходит на Xəzər TV с 2019 года и является одним из самых популярных комедийных шоу Азербайджана. Каждую неделю миллионы зрителей смотрят шоу по телевизору и на YouTube.",
    about_p2: "Шоу строится на юмористических сценках, сатирических монологах и актуальных социальных темах. Каждый выпуск дарит зрителям смех, энергию и позитивное настроение.",
    stat_since: "С года",
    stat_eps: "Эпизодов",
    stat_actors: "Актёров",
    ...
    ticket_desc: "Станьте живым зрителем на съёмках KOMEDİXANA! Заполните форму ниже, и мы свяжемся с вами.",
  ...
  en: {
    ...
    about_p1: "Broadcasting on Xəzər TV since 2019, Komedixana is one of Azerbaijan's most popular comedy shows. Every week, millions of viewers watch the show on TV and YouTube.",
    about_p2: "The show features humorous sketches, satirical monologues and topical social themes. Every episode brings laughter, energy and positivity to audiences.",
    stat_since: "Since",
    stat_eps: "Episodes",
    ...
    ticket_desc: "Join KOMEDIXANA as a live audience member! Fill in the form below and we will contact you.",
    form_name: "Full Name",
...
/* ---------- ACTORS DATA ---------- */
const actors = [
  { key: "enver", name: "Ənvər Abbasov", photo: "assets/actors/enver-abbasov.png", insta: "enverabbasov.official" },
  { key: "hesret", name: "Həsrət Həsrətov", photo: "assets/actors/hesret-hesretov.png", insta: "hasret.hasretov.official" },
  { key: "xeyale", name: "Xəyalə Quliyeva", photo: "assets/actors/xeyale-quliyeva.png", insta: "xeyalequliyevaofficial" },
  { key: "zarina", name: "Zarina Qurbanova", photo: "assets/actors/zarina-qurbanova.png", insta: "zarina_qubranova_" },
  { key: "cavidan", name: "Cavidan Novruz", photo: "assets/actors/cavidan-novruz.png", insta: "cavidan__novruz" },
  { key: "melek", name: "Mələk Kazımova", photo: "assets/actors/melek-kazimova.jpg", insta: "melek_kazimova" },
  { key: "mensur", name: "Mənsur Şərif", photo: "assets/actors/mensur-serif.png", insta: "mensur_sherif" },
  { key: "ilkin", name: "İlkin Misgərli", photo: "assets/actors/ilkin-misgerli.png", insta: "ilkinmisgerli" },
  { key: "elgun", name: "Elgün Hüseynov", photo: "assets/actors/elgun-huseynov.png", insta: "elgunhuseynov.eh" },
  { key: "gulzar", name: "Gülzar Əşrəfova", photo: "assets/actors/gulzar-eshrefova.png", insta: "gulzar_official" },
];
...
function applyLang(lang) {
  ...
  if (phoneInput) phoneInput.placeholder = lang === "ru" ? "Ваш номер телефона" : lang === "en" ? "Your phone number" : "+994 XX XXX XX XX";
  ...
  if (modalBackdrop.classList.contains("open")) {
    const currentKey = document.getElementById("modalPhoto").src.split("/").pop().split(".")[0];
    const actor = actors.find((a) => a.photo.includes(currentKey));
    if (actor) {
      const females = ["xeyale", "zarina", "melek", "gulzar"];
      const roleText = {
        az: females.includes(actor.key) ? "KOMEDİXANA Aktrisası" : "KOMEDİXANA Aktyoru",
...
function buildModalNav(currentKey) {
  ...
  actors.forEach((actor) => {
    ...
    btn.className = "modal-cast-thumb" + (actor.key === currentKey ? " active" : "");
    btn.innerHTML = `
...
function openModal(key) {
  ...
  document.getElementById("modalInstaLink").href = `https://www.instagram.com/${actor.insta}/`;
...
async function loadYoutubeVideos() {
  ...
  try {
    ...
    const sunday   = data.items[0];
    const saturday = data.items[1];
    ...
    // Əvvəlki Bölümlər: qalan videolar
    if (!grid) return;
    grid.innerHTML = "";
    data.items.slice(2).forEach((item, i) => {
      ...
      card.innerHTML = `
        ...
        <div class="episode-meta">
          ...
          <div class="episode-name">${item.snippet.title}</div>
        </div>
      `;
      grid.appendChild(card);
    });
